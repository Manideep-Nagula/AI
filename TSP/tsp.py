...
def find_MST(graph, nodes):
    ...
    mst_path[nodes[0]] = -1 # First node is always the root
...
if __name__ == '__main__': 
    ...
    if len(graph) != 0:
        ...
        if check_connected(graph, list(graph.keys())[0]):
            ...
            path, generated_nodes, expanded_nodes = optimal_tsp(graph, V) 
            print(path, '\n')
